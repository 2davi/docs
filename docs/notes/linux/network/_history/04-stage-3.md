...
> 개인 노트: Unknown Unicast Flooding이라고 했는데, 사실상 브로드캐스트 요청을 보내는 거 아닌가?
>
...
1. **브리지의 flooding vs unicast 전달** → 1부 학습하는 스위치
2. **`src MAC`+입력 포트로 학습하는 원리** → 1부 어떻게 학습하나
3. **flooding된 프레임을 남의 NIC가 버리는 판단의 주체·시점** → 2부 하드웨어 필터
4. **가상화에서 무작위 모드가 필요한 이유** → 2부 가상화의 딜레마
5. **무작위 모드 미설정 시 나가는/들어오는 비대칭과 증상** → 2부 방향의 비대칭
6. **L2 loop가 멈추지 않는 이유(TTL 부재)와 무작위 모드가 storm을 키우는 이유** → 3부 Broadcast Storm
7. **VLAN 태그를 누가 붙이고 떼나, Access vs Trunk** → 4부 VLAN 802.1Q
8. **클러스터 노드 간 Trunk / 종단 VM Access** → 4부 클러스터 케이블
9. **pfSense가 Trunk여야 하는 이유, "VM=Access" 법칙의 상대성** → 5부 역할이 포트를 결정한다
