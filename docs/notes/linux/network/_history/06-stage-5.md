...
## 04. Split-Brain을 막는 세 층위 ─ Quorum · Fence · Watchdog
...
## 06. Control Plane vs Data Plane
...
## 08. 디스크는 어디 있어야 하는가 ─ 세 시나리오와 RPO
...
1. **(a)/(b) 구별 불가 = FLP 불가능성, 결정론 포기** → 02
2. **양쪽 동시 실행 = split-brain, CAP의 CP 선택** → 03
3. **합의 메커니즘 = Corosync Totem 단방향 토큰 링** → 05 *(+ 04 quorum/fence/watchdog 세 층)*
4. **멤버십 협상 중 VM = Control/Data Plane 분리** → 06 *(+ 07 HA Manager·failover 흐름)*
5. **디스크 위치 세 시나리오 = 복제 여부·RPO·SPOF, Ceph로 환원** → 08~09
