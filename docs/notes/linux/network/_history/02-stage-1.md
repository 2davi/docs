...
1. **라우팅 테이블 두 줄 중 직접/경유 구분과 결정적 단어** → [2. 라우팅 테이블 읽는 법](#_2-라우팅-테이블-읽는-법-via가-모든-것을-가른다)
2. **Proxmox 노드가 8.8.8.8로 보낼 때 걸리는 줄과 경로** → [2. Longest Prefix Match](#longest-prefix-match-어느-줄에-걸리는가)
3. **같은 8.8.8.8인데 Debian VM과 Proxmox의 게이트웨이가 다른 이유** → [3. 라우팅은 각 노드의 관점](#_3-같은-8-8-8-8-다른-게이트웨이-라우팅은-각-노드의-관점)
4. **브리지·커널·캡슐화·dst MAC 한 줄 요약** → [7. 브리지와 커널의 분업](#_7-브리지와-커널의-분업-stage-1의-정수)
