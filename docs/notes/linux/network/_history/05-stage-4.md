...
### 관리망은 최후의 보루 ─ Out-of-Band Management
...
## 05. Defense in Depth ─ 대체가 아니라 양보
...
## 07. 통신의 방향성 ─ 신뢰 비대칭(Trust Asymmetry)
...
## 10. pfSense 규칙 평가 순서 ─ top-down, first-match
...
## 12. Unbound DNS Resolver ─ 내부 도메인
...
1. **분리의 동기 네 갈래** → 01 왜 분리하는가
2. **동기 × 유형 매트릭스, OOB·Corosync 황금률** → 02 두 축
3. **과분리의 비용(인프라·운영·진단 복잡도)** → 03 분리는 공짜가 아니다
4. **적정 분리도의 다섯 원칙 + 대체가 아닌 양보** → 04~05
5. **물리 vs 논리, trust boundary가 하드웨어냐 소프트웨어냐, 하이브리드** → 06
6. **신뢰 비대칭과 default deny + explicit allow** → 07
7. **매개체 카탈로그와 언제 필요한가** → 08
