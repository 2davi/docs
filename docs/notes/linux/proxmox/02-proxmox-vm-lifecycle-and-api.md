---
title: "Proxmox VE VM 라이프사이클 & REST API 심화 학습"
date: 2026-04-07
lastmod: 2026-04-07
author: "Davi"
description: "VM 생성/삭제/복제/스냅샷/백업/복구의 CLI 조작과 Proxmox REST API, 인증 체계, 비동기 태스크, QEMU Guest Agent, Cloud-Init까지 CMP 개발자 관점에서 정리한다."
slug: "proxmox-vm-lifecycle-and-api"
...
### 4.1 Full Clone vs Linked Clone
...
스냅샷은 **특정 시점의 VM 상태를 캡처**하는 것이다. 디스크의 상태, 설정, 그리고 선택적으로 메모리(RAM) 상태까지 보존한다. 마치 게임에서 "세이브 포인트"를 찍는 것과 같다.

### 5.2 QCOW2 스냅샷 체인 (Snapshot Chain)
...
**성능 영향:** 체인이 길어질수록, 특정 블록을 읽을 때 여러 레이어를 거슬러 올라가야 하므로 I/O 성능이 저하된다. 스냅샷은 **단기 롤백 용도**로 사용하고, 작업이 끝나면 삭제하는 것이 원칙이다. 장기 보관 목적이라면 백업(vzdump)을 사용하라.

### 5.3 RAM State 포함 여부 (`--vmstate`)
...
이것은 반드시 기억해야 할 원칙이다. 스냅샷은 **같은 디스크 위에** 존재한다. 디스크가 물리적으로 고장 나면 스냅샷도 함께 소실된다. 반면 백업(vzdump)은 별도의 스토리지에 독립적인 파일로 저장된다.
...
| 알고리즘      | 속도 | 압축률 | 특징                                         |
| ------------- | ---- | ------ | -------------------------------------------- |
| `lzo`         | 빠름 | 낮음   | CPU 부하 최소, 빠른 백업/복원 우선 시        |
| `gzip`        | 느림 | 높음   | 전통적, 호환성 좋음                          |
| `zstd` (권장) | 빠름 | 높음   | 속도와 압축률 모두 우수. Proxmox 기본 권장값 |

`zstd`는 Facebook이 개발한 압축 알고리즘으로, `gzip` 수준의 압축률을 `lzo` 수준의 속도로 달성한다. 특별한 이유가 없다면 `zstd`를 사용하라.
