...
도구를 점프박스에 몰아 두는 데는 뒤 페이즈로 이어지는 근거가 있다. 인증서 서명은 CA 개인키가 있는 자리에서만 일어난다. 그 개인키를 점프박스에만 두면, _서명이라는 민감한 조작이 클러스터 노드가 아니라 관리 거점 한 곳에 갇힌다._ 이 배치의 근거는 [PKI와 TLS 신뢰 사슬](./pki-tls-trust-chain) 문서에서 다룬다.
